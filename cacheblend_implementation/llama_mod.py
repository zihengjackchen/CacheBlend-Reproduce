...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        # @NOTE (Jack): Collect important tokens from each layer
        self.rotary_embeddings = self.rotary_emb.cos_sin_cache

        attn_output = self.attn(q, k, v, kv_cache, attn_metadata,
                                status, cache_fuse_metadata, old_kv,
                                self.kv_scale)
        
        if cache_fuse_metadata['collect'] and cache_fuse_metadata["imp_indices"] != None:
            # @NOTE (Jack): Collect important tokens from each layer
            # self.imp_tokens = kv_cache["imp_indices"].clone()
            self.imp_indices = cache_fuse_metadata["imp_indices"].clone() 
