...
select_imp = "select_full_recompute2"
# select_imp = "select_5_21"

for sample_idx in range(1,11):
    for run_num in range(1,11):
        f = open(f"/u/zihengc2/multistage-sd/CacheBlend/inputs/{sample_idx}.json")
        ex = json.load(f)
        ...
        # imp_indices_cpu = cache_fuse_metadata['imp_indices'].cpu().numpy().tolist()
        # Prepare dictionary
        # data = {
        #     "imp_indices": imp_indices_cpu,
        #     "ttft_full_recompute": output[0].metrics.first_token_time-output[0].metrics.first_scheduled_time,
        #     "output_text": output[0].outputs[0].text,
        #     "imp_indices_unsorted": cache_fuse_metadata['imp_indices_unsorted'].cpu().numpy().tolist(),
        #     "imp_indices_last": cache_fuse_metadata['imp_indices_last']
        # }

        # Save as JSON
        # os.makedirs(f"/u/zihengc2/multistage-sd/CacheBlend/{select_imp}", exist_ok=True)
        # with open(f"/u/zihengc2/multistage-sd/CacheBlend/{select_imp}/output_sample_{sample_idx}_run_{run_num}.json", "w") as f:
